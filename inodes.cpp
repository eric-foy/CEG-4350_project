...
uint Inodes::create(FileVolume *pfv, uint nBegin, uint nInodes, uint iHeight) {
    ...
    fv->superBlock.iDirect =
        iHeight - 1 - 1 - iIndirect;  // see xType, xFileSize
...
uint *Inodes::getInode(uint in, uint *ne) {
    uint nblock =
        fv->superBlock.nBlockBeginInodes + in / fv->superBlock.inodesPerBlock;
...
uint Inodes::getType(uint in) { return getEntry(in, xType); }

uint Inodes::setType(uint in, uint tp) { return setEntry(in, xType, tp); }

/* pre:: 0 < in < nInodes ;; post:: Return the size of file whose
 * i-number is in. */

uint Inodes::getFileSize(uint in) { return getEntry(in, xFileSize); }
...
uint Inodes::addBlockNumber(uint in, uint bn) {
    return in == 0 || in >= fv->superBlock.nInodes || bn == 0 ||
                   bn >= fv->superBlock.nTotalBlocks
               ? 0
...
uint Inodes::getBlockNumber(uint in, uint nth) {
    ...
    if (nth >= nthmax) return 0;

    uint iDirect = fv->superBlock.iDirect;
    if (nth < iDirect) return pin[nth];
    ...
    uint iIndirectTwo = iIndirectOne + bnpb * bnpb;
    if (nth < iIndirectTwo)
        return getBlockNumberDoubleIndirect(pin[iDirect + 1],
                                            nth - iIndirectOne);

    uint iIndirectThree = iIndirectTwo + bnpb * bnpb * bnpb;
    if (nth < iIndirectThree)
        return getBlockNumberTripleIndirect(pin[iDirect + 2],
                                            nth - iIndirectTwo);
...
uint Inodes::show(uint in, FILE *my_stream) {
    uint *pin = getInode(in, 0), x = 0;
    fprintf(my_stream, "inode #%d == [", in);
    for (; x < xFileSize; x++) {
