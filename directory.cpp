...
uint Directory::lsPrivate(uint in, uint printfFlag, FILE *my_stream)
{
  uint nFiles = 0;
  ...
  for (byte *bp = 0; (bp = d->nextName()); nFiles++) {
    ...
    if (printfFlag) {
      ...
      fprintf(my_stream, "%7d %crw-rw-rw-    1 yourName yourGroup %7d Jul 15 12:34 %s\n",
...
uint Directory::ls(FILE *my_stream)
{
  return lsPrivate(nInode, 1, my_stream);	// 1 ==> printf it
}
